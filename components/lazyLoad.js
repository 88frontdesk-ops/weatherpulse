function loadLib(urls) {
  return (
    ...
    Promise.all(
      urls.map(
        (url) => (
          window._loadedLibs[url] ||
            (window._loadedLibs[url] = new Promise((resolve, reject) => {
              ...
              (isCss
                ? ((tag.rel = "stylesheet"),
                  (tag.type = "text/css"),
                  (tag.href = url))
                : ((tag.src = url), (tag.type = "text/javascript")),
                (tag.onload = () => resolve()),
...
function loadCharts() {
  return loadLib("/libraries/chart.umd.min.js").then(() =>
    loadLib([
      "/libraries/chartjs-chart-matrix.min.js",
      "/libraries/chartjs-plugin-datalabels.min.js",
      "/libraries/chartjs-adapter-date-fns.js",
    ]),
  );
}
