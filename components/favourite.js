const favourite = () => {
  ((favIcon_current.style.display = "none"),
    ...
    (favIcon_lunar.style.display = "none"),
    chrome.storage.local.get(["setAsHomepage", "latlong", "citys"], (data) => {
      "report" == data.setAsHomepage
        ? (displayModal(), currentPage())
            ...
            : "solar" == data.setAsHomepage
              ? (closeAllPopup(),
                ...
                (favIcon_solar.style.backgroundImage =
                  'url("/images/favourite-active.svg")'),
                solarIcon.classList.add("sub_menu_icon_active_Class"),
                solarIcon.classList.add("sub_menu_current_icon_Class"),
                solarSub.classList.add("sub_menu_current_Class"))
              : "aqi" == data.setAsHomepage
                ? (aqi_api(data.latlong, updateTime),
                  closeAllPopup(),
                  displayModal(),
