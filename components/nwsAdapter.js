(() => {
  ...
  const fetchOpenMeteo = async (latitude, longitude, timezone) => {
    ...
    const localTimeToIso = (value) => {
      if (!value) return "";
      const localAsUtc = Date.parse(`${value}:00Z`);
      return Number.isFinite(localAsUtc)
        ? new Date(localAsUtc - offsetSeconds * 1000).toISOString()
        : "";
    };
    ...
    const hours = times.map((time, index) => {
      ...
      return {
        ...
        precipitationChance: clamp01(
          (hourly.precipitation_probability?.[index] ?? 0) / 100,
        ),
    ...
    const days = (daily.time || []).map((date, index) => {
      ...
      return {
        ...
        maxUvIndex: daily.uv_index_max?.[index] ?? 0,
        sunrise: localTimeToIso(daily.sunrise?.[index]),
        sunset: localTimeToIso(daily.sunset?.[index]),
        sunriseCivil: localTimeToIso(daily.sunrise?.[index]),
        sunsetCivil: localTimeToIso(daily.sunset?.[index]),
        sunriseAstronomical: localTimeToIso(daily.sunrise?.[index]),
        sunsetAstronomical: localTimeToIso(daily.sunset?.[index]),
        sunriseNautical: localTimeToIso(daily.sunrise?.[index]),
        sunsetNautical: localTimeToIso(daily.sunset?.[index]),
        solarNoon: localTimeToIso(daily.sunrise?.[index]),
        solarMidnight: localTimeToIso(daily.sunset?.[index]),
        moonrise: localTimeToIso(daily.moonrise?.[index]),
        moonset: localTimeToIso(daily.moonset?.[index]),
    ...
    return {
      currentWeather: {
        ...
        pressure: current.pressure_msl ?? 1013.25,
        pressureTrend: "",
  ...
  const fetchOpenMeteoHistoricalDay = async (
    latitude,
    longitude,
    timezone,
    date,
  ...
  const fetchNwsForecast = async (latitude, longitude) => {
    try {
      const headers = {
        ...
        "User-Agent": "Weather Pulse weather extension",
      };
      ...
      return Array.isArray(data?.properties?.periods)
  ...
  const applyNwsDetailedForecasts = (weather, periods) => {
    ...
    weather.forecastDaily.days = weather.forecastDaily.days.map((day) => {
      ...
      const daytimeForecast = dayPeriod?.detailedForecast
        ? {
            ...day.daytimeForecast,
            description: dayPeriod.detailedForecast,
            detailedForecast: dayPeriod.detailedForecast,
          }
        : day.daytimeForecast;
      const overnightForecast = nightPeriod?.detailedForecast
        ? {
            ...day.overnightForecast,
            description: nightPeriod.detailedForecast,
      ...
      return {
        ...
        nwsDetailedForecast: {
          ...
          night: nightPeriod?.detailedForecast || "",
        },
      };
    });
    return weather;
  ...
  const fetchNwsAlerts = async (latitude, longitude) => {
    try {
      const data = await requestJson(
        ...
        {
          Accept: "application/geo+json",
          "User-Agent": "Weather Pulse extension",
        },
      );
      return {
        alerts: (data.features || []).map((feature) => {
          ...
          return {
            ...
            description: p.event || "Weather alert",
