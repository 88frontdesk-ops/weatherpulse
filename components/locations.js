...
const loadLocations = () => {
      ...
      (data) => {
        if (
          ...
          for (let i = 0; i < selectedLocation.length; i++)
            ((city = selectedLocation[i].split(",")[0]),
              (country = selectedLocation[i].split(",")[1]),
              (lat = selectedLocation[i].split(",")[2]),
              (lng = selectedLocation[i].split(",")[3]),
              (timezone = tzlookup(lat, lng)),
              (locationListID = "locationListID"),
              (locationListClass = selectedLocation[i].split(",")[6]),
              selectedLocationNew.push(
                city +
                  "," +
                  country +
                  "," +
          ...
          chrome.storage.local.set({
            ...
            selectedLocation: selectedLocationNew,
          });
        }
      },
    );
  ...
  selectedLocations = (selectedLocation) => {
    ...
    (selectedLocation.unshift(
      ...
      chrome.storage.local.get(
        ["selectedLocationNumber", "selectedLocationMax"],
        (data) => {
    ...
    let lat = latlong.split(",")[0],
      lng = latlong.split(",")[1];
    (document
      .getElementById("locationNameGroup")
      .appendChild(
        createLocationList(
          citys,
          country,
      ...
      (data) => {
        if (
          ...
          ((removeAllChildNodes = (parent) => {
            ...
            removeAllChildNodes(document.getElementById("locationNameGroup")),
            (selectedLocation = data.selectedLocation));
          for (let i = 0; i < data.selectedLocationNumber; i++)
            ((city = data.selectedLocation[i].split(",")[0]),
              (country = data.selectedLocation[i].split(",")[1]),
              (lat = data.selectedLocation[i].split(",")[2]),
              (lng = data.selectedLocation[i].split(",")[3]),
              (timezone = data.selectedLocation[i].split(",")[4]),
              (eClass = data.selectedLocation[i].split(",")[6]),
              "locationDefaultTitle" == eClass
                ? ((temp =
                    "c" == data.setSettingFC
                      ? k2c(temperature) + "°"
                      : k2f(temperature) + "°"),
  ...
  createLocationList = (
    ...
    let iconAndTitleContainer = document.createElement("span"),
      imageCity = document.createElement("img");
    imageCity.setAttribute("id", "locationNameIcon");
    let spanCity = document.createElement("span"),
      spanTemperature = document.createElement("span");
    return (
      spanTemperature.setAttribute("class", "temperatureLocations"),
      ...
      iconAndTitleContainer.setAttribute("class", "iconAndTitleContainer"),
      iconAndTitleContainer.appendChild(imageCity),
      iconAndTitleContainer.appendChild(spanCity),
      wrapCity.appendChild(iconAndTitleContainer),
      wrapCity.appendChild(spanTemperature),
      ...
      (imageCity.onclick = () => {
        ...
        chrome.storage.local.get(
          ["selectedLocationNumber", "selectedLocationMax"],
          (data) => {
            "locationDefaultTitle" !== defaultStyle[6] &&
              ...
              data.selectedLocationNumber >= data.selectedLocationMax &&
                ((searchBoxGeocoder.style.display = "block"),
                (myLocationIp.style.display = "block"),
                (maximumNumber.style.display = "none")));
          },
        );
      }),
      (spanCity.onclick = () => {
        ...
        ((preloaderLocation.style.display = "block"),
          (preloaderLocation.style.opacity = 0.9),
          closeAddLocation(),
          (clickedIndex = [...wrapCity.parentElement.children].indexOf(
            wrapCity,
          )));
        for (let i = 0; i < selectedLocation.length; i++) {
          let splitResult = selectedLocation[i].split(",");
